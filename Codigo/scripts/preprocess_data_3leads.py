...
os.makedirs("./3lead_data", exist_ok=True)
with open("./3lead_data/train_REC", 'w+') as f:
    for index, row in Y_train.iterrows():
        f.write(row['filename'] + "\n")
    Y_train.drop(columns=['filename', "OTHER"]).to_csv("./3lead_data/train_db.csv")

with open("./3lead_data/validation_REC", 'w+') as f:
    for index, row in Y_validation.iterrows():
        f.write(row['filename'] + "\n")
    Y_validation.drop(columns=['filename', "OTHER"]).to_csv("./3lead_data/validation_db.csv")

with open("./3lead_data/test_REC", 'w+') as f:
    for index, row in Y_test.iterrows():
        f.write(row['filename'] + "\n")
    Y_test.drop(columns=['filename', "OTHER"]).to_csv("./3lead_data/test_db.csv")

os.system("python3 ./sergio/generate_h5.py ./3lead_data/test_REC ./3lead_data/test.hdf5 --root_dir ./ptbxl")
os.system("python3 ./sergio/generate_h5.py ./3lead_data/train_REC ./3lead_data/train.hdf5 --root_dir ./ptbxl")
os.system("python3 ./sergio/generate_h5.py ./3lead_data/validation_REC ./3lead_data/validation.hdf5 --root_dir ./ptbxl  ")
